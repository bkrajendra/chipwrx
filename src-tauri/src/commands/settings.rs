...
pub struct GlobalSettingsPatch {
    ...
    pub updates: Option<crate::core::settings::UpdateSettings>,
    pub onboarding_completed: Option<bool>,
    pub privacy_notice_acknowledged: Option<bool>,
}
...
pub async fn settings_set_global(
    ...
    if let Some(v) = patch.updates {
        guard.updates = v;
    }
    if let Some(v) = patch.onboarding_completed {
        guard.onboarding_completed = v;
    }
    if let Some(v) = patch.privacy_notice_acknowledged {
        guard.privacy_notice_acknowledged = v;
    }
