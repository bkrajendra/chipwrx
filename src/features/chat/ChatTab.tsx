// Canvas "Chat" tab (`FR-UI-3`, default tab) — conversation, streaming, tool cards.
...
function ToolCallCard({ block }: { block: Extract<Block, { kind: "tool" }> }) {
  ...
  return (
    <div className={`rounded border px-2.5 py-1.5 text-xs ${isError ? "border-red-300 dark:border-red-800" : "border-neutral-200 dark:border-neutral-800"}`}>
      <button type="button" onClick={() => setExpanded((v) => !v)} aria-expanded={expanded} className="flex w-full items-center justify-between text-left">
        <span className="font-mono font-medium">{block.name}</span>
        <span className="text-neutral-500 dark:text-neutral-400">{block.result ? (isError ? "failed" : "done") : "running…"}</span>
      </button>
      {expanded && (
        <div className="mt-1.5 space-y-1 border-t border-neutral-200 pt-1.5 dark:border-neutral-800">
          {block.input !== undefined && (
            <pre className="overflow-x-auto whitespace-pre-wrap text-[11px] text-neutral-600 dark:text-neutral-400">{JSON.stringify(block.input, null, 2)}</pre>
          )}
          {block.result && <p className="whitespace-pre-wrap text-[11px] text-neutral-600 dark:text-neutral-400">{block.result.full ?? block.result.summary}</p>}
        </div>
      )}
    </div>
  );
...
function BlockView({ block }: { block: Block }) {
  switch (block.kind) {
    ...
    case "compact":
      return <p className="text-center text-[11px] text-neutral-500 dark:text-neutral-400">— context compacted —</p>;
    case "result":
...
function PromptBubble({ text }: { text: string }) {
  return <div className="ml-auto max-w-[85%] rounded-lg bg-neutral-900 px-3 py-2 text-sm text-neutral-50 dark:bg-neutral-100 dark:text-neutral-900">{text}</div>;
}
  ...
  return (
    <div className="space-y-2">
      ...
      {turn.toolCalls.map((tc) => (
        <div key={tc.toolUseId} className={`rounded border px-2.5 py-1.5 text-xs ${tc.isError ? "border-red-300 dark:border-red-800" : "border-neutral-200 dark:border-neutral-800"}`}>
...
function LiveTurnCard({ live, onOpenChanges }: { live: LiveTurn; onOpenChanges: (turnId: string) => void }) {
  return (
    <div className="space-y-2">
      ...
      {live.blocks.length === 0 && live.running && <p className="text-sm text-neutral-500 dark:text-neutral-400">Thinking…</p>}
      {live.blocks.map((b, i) =>
        b.kind === "result" ? (
          <ResultFooter key={i} numTurns={b.numTurns} durationMs={b.durationMs} totalCostUsd={b.totalCostUsd} isError={b.isError} onOpenChanges={live.turnId ? () => onOpenChanges(live.turnId) : undefined} />
        ) : (
...
/** `FR-CHAT-8`: these four are intercepted client-side; everything else starting with `/`
 * (skills, custom commands) is passed through to the CLI unchanged. */
const APP_OWNED_SLASH_COMMANDS = new Set(["/build", "/upload", "/monitor", "/doctor"]);
...
export function ChatTab({
  ...
  onOpenChanges: (turnId: string) => void;
  onBuild: () => void;
  onUpload: () => void;
  onOpenMonitor: () => void;
  onOpenDoctor: () => void;
}) {
  const { history, live, loadingHistory, error, send, stop } = chat;
  ...
  const handleSend = (text: string, attachments: string[]) => {
    ...
    if (command && APP_OWNED_SLASH_COMMANDS.has(command)) {
      if (command === "/build") onBuild();
      else if (command === "/upload") onUpload();
      else if (command === "/monitor") onOpenMonitor();
      else if (command === "/doctor") onOpenDoctor();
      return;
    }
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="mx-auto max-w-4xl space-y-4">
          ...
          {history.map((t) => (
            <TurnHistoryCard key={t.turnId} turn={t} onOpenChanges={onOpenChanges} />
          ))}
          {live && <LiveTurnCard live={live} onOpenChanges={onOpenChanges} />}
          <div ref={bottomRef} />
        </div>
      </div>
