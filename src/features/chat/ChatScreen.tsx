...
function TurnHistoryCard({ turn }: { turn: TurnRecord }) {
  return (
    <div className="space-y-2">
      ...
      {turn.result && (
        <ResultFooter numTurns={turn.result.numTurns} durationMs={turn.result.durationMs} totalCostUsd={turn.result.totalCostUsd} isError={turn.result.isError} />
      )}
    </div>
  );
}

function LiveTurnCard({ live }: { live: LiveTurn }) {
  return (
    <div className="space-y-2">
...
export function ChatScreen({ workspaceId, workspaceName }: { workspaceId: string; workspaceName: string }) {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="mx-auto max-w-2xl space-y-4">
          ...
          {history.map((t) => (
            <TurnHistoryCard key={t.turnId} turn={t} />
          ))}
          {live && <LiveTurnCard live={live} />}
          <div ref={bottomRef} />
        </div>
      </div>
